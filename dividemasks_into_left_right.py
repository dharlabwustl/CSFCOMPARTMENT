        ...
        for img_idx in range(numpy_image.shape[2]):
            if img_idx>0 and img_idx < numpy_image.shape[2]:
                ...
                if os.path.exists(this_npyfile):
                    ...
                    x_points2=x_points2[:,0]
                    y_points2=y_points2[:,0]
                    filtered_slice=filter_clusters_by_distance_and_size(Mask_filename_data_np[:,:,img_idx], (int(y_points2[511]),int(x_points2[511])),(int(y_points2[0]),int(x_points2[0])), distance_thresh=25, size_thresh=100)
